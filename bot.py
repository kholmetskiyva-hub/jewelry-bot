"""
JJewelry Bot — АКТУАЛЬНАЯ ВЕРСИЯ.

VERSION_ADMIN_GROUPS_BROADCAST_ADMINS_FIX_2026_06_25

В этой версии реализовано:
1) «Все записи» группируются по мастер-классам.
2) Кнопка «Добавить мастер-класс» убрана из главной админ-панели.
   Добавление осталось внутри «Расписание встреч».
3) «Администраторы»: показать текущих, добавить нового, удалить старого.
4) «Рассылка»:
   - всем пользователям бота;
   - по конкретному мастер-классу;
   - тем, кто подписался на бот, но ни разу не записался;
   - всем, кто когда-либо записывался.
5) Перенос записи в админ-панели.
6) Запрет повторной записи одного пользователя на один и тот же мастер-класс.
"""
...
VERSION = "VERSION_ADMIN_GROUPS_BROADCAST_ADMINS_FIX_2026_06_25"
...
def all_known_user_ids():
    settings = load_settings()
    users = set()
    for uid in settings.get("known_users", []):
        try:
            users.add(int(uid))
        except Exception:
            pass
    for b in load_bookings():
        try:
            users.add(int(b.get("user_id")))
        except Exception:
            pass
    return users


def users_with_any_booking():
    users = set()
    for b in load_bookings():
        try:
            users.add(int(b.get("user_id")))
...
def users_never_booked():
    return all_known_user_ids() - users_with_any_booking()
...
def admin_keyboard():
    return ReplyKeyboardMarkup(
        [
            ...
            ["❌ Отменить запись", "🔄 Перенести запись"],
...
def broadcast_keyboard():
    return ReplyKeyboardMarkup(
        [
            ["💌 Всем пользователям бота"],
            ["💎 По конкретному мастер-классу"],
            ["🌸 Подписались, но не записались"],
            ["✨ Кто когда-либо записывался"],
            ["⚙️ Админ-панель"],
        ],
        resize_keyboard=True,
    )
...
async def admin_all_bookings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    grouped = {}
    for b in bookings:
        key = int(b.get("class_id", 0))
        grouped.setdefault(key, []).append(b)

    classes_by_id = {int(mc["id"]): mc for mc in load_classes()}
    text = "📋 Все записи по мастер-классам 💎

"

    for class_id, items in grouped.items():
        mc = classes_by_id.get(class_id, {})
        title = mc.get("title") or items[0].get("class_title", "Мастер-класс")
        date = mc.get("date") or items[0].get("class_date", "")
        spots_total = mc.get("spots", "—")
        text += (
            f"💎 {title}
"
            f"📅 {fmt_date(date)}
"
            f"👩 Записано: {len(items)} / {spots_total}
"
        )
        for b in items:
            text += f"  • #{b['id']} {b.get('name','')} | {b.get('phone','')} | {b.get('contact','')}
"
        text += "
"
...
async def choose_broadcast_class(update: Update, context: ContextTypes.DEFAULT_TYPE):
    classes = load_classes()
    if not classes:
        await update.message.reply_text("🌸 Мастер-классов пока нет.", reply_markup=admin_keyboard())
        return

    rows = [[f"💎 Рассылка МК #{mc['id']} — {mc['title'][:25]}"] for mc in classes]
    rows.append(["⚙️ Админ-панель"])

    await update.message.reply_text(
        "💎 Выберите мастер-класс для рассылки:",
        reply_markup=ReplyKeyboardMarkup(rows, resize_keyboard=True),
    )


async def start_broadcast_text(update: Update, context: ContextTypes.DEFAULT_TYPE, target_type: str, class_id=None):
    if not is_admin(update.effective_user.id):
        return
    ...
    names = {
        ...
        "class": "записанным на выбранный мастер-класс",
        "never_booked": "тем, кто подписался, но ещё не записывался",
        "ever_booked": "всем, кто когда-либо записывался",
    }

    await update.message.reply_text(
        f"💌 Введите текст рассылки для группы: {names.get(target_type, target_type)} ✨",
        reply_markup=cancel_keyboard(),
    )


async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    target = context.user_data.get("broadcast_target", {"type": "all"})
    ...
    if target_type == "all":
        recipients = all_known_user_ids()
    elif target_type == "class":
        for b in load_bookings():
            if b.get("status") == "confirmed" and int(b.get("class_id", 0)) == int(class_id):
                recipients.add(int(b["user_id"]))
    elif target_type == "never_booked":
        recipients = users_never_booked()
    elif target_type == "ever_booked":
        recipients = users_with_any_booking()
...
"
    for admin_id in admins:
        marker = "главный" if int(admin_id) == MAIN_ADMIN_ID else "дополнительный"
        text += f"• {admin_id} — {marker}
"

    rows = [["➕ Добавить администратора"]]
    for admin_id in admins:
        if int(admin_id) != MAIN_ADMIN_ID:
            rows.append([f"🗑 Удалить администратора {admin_id}"])
    rows.append(["⚙️ Админ-панель"])
...
async def save_new_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    if not text.isdigit():
        await update.message.reply_text("🌸 Введите ID только цифрами.", reply_markup=cancel_keyboard())
        return

    new_id = int(text)
    settings = load_settings()
    admins = []
    for x in settings.get("admins", []):
        try:
            admins.append(int(x))
        except Exception:
            pass

    if new_id != MAIN_ADMIN_ID and new_id not in admins:
...
async def remove_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, admin_id: int):
    if not is_admin(update.effective_user.id):
        return
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "add_admin":
        await save_new_admin(update, context, text)
        return
    ...
    if is_admin(uid):
        ...
        if text == "💌 Всем пользователям бота":
            await start_broadcast_text(update, context, "all")
            return

        if text == "💎 По конкретному мастер-классу":
            await choose_broadcast_class(update, context)
            return

        if text.startswith("💎 Рассылка МК #"):
            try:
                class_id = int(text.split("МК #", 1)[1].split(" ", 1)[0])
                await start_broadcast_text(update, context, "class", class_id)
            except Exception:
                await update.message.reply_text("🌸 Не смогла определить мастер-класс.", reply_markup=admin_keyboard())
            return

        if text == "🌸 Подписались, но не записались":
            await start_broadcast_text(update, context, "never_booked")
            return

        if text == "✨ Кто когда-либо записывался":
            await start_broadcast_text(update, context, "ever_booked")
            return
        ...
        if text.startswith("🗑 Удалить администратора "):
            try:
                admin_id = int(text.split("администратора ", 1)[1])
                await remove_admin(update, context, admin_id)
            except Exception:
                await update.message.reply_text("🌸 Не смогла определить администратора.", reply_markup=admin_keyboard())
            return
