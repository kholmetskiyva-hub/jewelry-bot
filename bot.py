...
2) Кнопка «Добавить мастер-класс» убрана из главной админ-панели.
   Добавление доступно внутри «Расписание встреч».
3) «Администраторы»: показать текущих, добавить нового, удалить старого.
4) «Рассылка»:
   - всем пользователям бота;
   - записанным на конкретный мастер-класс;
   - пользователям, которые открывали бот, но ни разу не записывались;
   - всем, кто когда-либо записывался.
5) Перенос записи в админ-панели.
6) Один пользователь не может записаться на один и тот же мастер-класс больше одного раза.
"""
...
VERSION = "VERSION_ADMIN_GROUPS_BROADCAST_ADMINS_FIX_2026_06_25_FINAL"
...
def get_admins():
    ...
    for item in settings.get("admins", []):
        try:
            admin_id = int(item)
            if admin_id != MAIN_ADMIN_ID and admin_id not in admins:
...
def free_spots(class_id: int) -> int:
    ...
    taken = sum(
        ...
        if int(b.get("class_id", 0)) == int(class_id)
...
async def admin_all_bookings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for b in bookings:
        ...
        grouped.setdefault(class_id, []).append(b)
...
async def remove_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, admin_id: int):
    ...
    for item in settings.get("admins", []):
        try:
            item_id = int(item)
            if item_id != int(admin_id):
                new_admins.append(item_id)
        except Exception:
            pass
    ...
    if is_admin(uid):
        ...
        if text.startswith("🗑 Удалить администратора "):
            try:
                ...
                await update.message.reply_text(
                    "🌸 Не смогла определить администратора.",
                    reply_markup=admin_keyboard(),
                )
            return
